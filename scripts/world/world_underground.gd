...
var easy_monster_scene = preload("res://scenes/actors/EasyMonster.tscn")
...
func _ready() -> void:
	...
	_spawn_easy_monster_if_needed()
	_spawn_food_if_needed()

func _handle_return_from_battle() -> void:
	if GameState.last_battle_result == "win":
		GameState.underground_easy_monster_defeated = true

	GameState.clear_battle_result()

func _spawn_easy_monster_if_needed() -> void:
	if GameState.underground_easy_monster_defeated:
		return

	if not GameState.should_spawn_underground_easy_monster:
		return

	var points = [
		spawn_a.global_position,
		spawn_b.global_position
	]

	var monster = easy_monster_scene.instantiate()
	monster.global_position = points[randi() % points.size()]
	add_child(monster)
...
func _is_valid_food_position(pos: Vector2) -> bool:
	...
	if pos.distance_to(spawn_a.global_position) < 70:
		return false

	if pos.distance_to(spawn_b.global_position) < 70:
		return false
